...
def dense_to_one_hot(labels_dense, num_classes):
    ...
    #print(labels_one_hot.flat[index_offset + labels_dense.ravel()])
...
with graph.as_default():
    ...
    learning_rate = tf.train.exponential_decay(LEARNING_RATE, global_step, 50, 0.65, staircase=True)
...
print('iter per epoch: %d' % iter_per_epoch)
...
with tf.Session(graph=graph, config=tf.ConfigProto(log_device_placement=True)) as sess:
  ...
  #  print('test_accuracy => %.3f, test_prediction => %.5f , learning_rate => %.5f' % (
  #  test_accuracy, test_prediction, learning_rate.eval()))
